...
class UserTest < ActiveSupport::TestCase
  ...
  test 'check that a user with the existing username cannot be save' do
    user = User.new( username: users(:one).username, email: @email, password: @password, password_confirmation: @password )
    refute user.save
  end

  test 'check that a user with the existing email cannot be save' do
    user = User.new( username: @username, email: users(:one).email, password: @password, password_confirmation: @password )
    refute user.save
  end
end
